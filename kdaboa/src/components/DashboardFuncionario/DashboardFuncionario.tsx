...
export default function DashboardFuncionario(props: any) {
  ...
  const NAVIGATION: Navigation = [
    { kind: 'header', title: 'Opções' },
    {
      segment: 'dashboard',
      title: 'Dados pessoais',
      icon: <Face />,
      children: [
        { segment: 'inicio', title: 'Inicio', icon: <Home /> },
        { segment: 'estabelecimento', title: 'Estabelecimento', icon: <House /> },
        { segment: 'endereco', title: 'Endereço', icon: <Map /> },
        { segment: 'contato', title: 'Contatos', icon: <Call /> },
        { segment: 'galeria', title: 'Galeria', icon: <Collections /> },
        { segment: 'info', title: 'Informações cadastrais', icon: <Person /> },
      ],
    },
    { kind: 'divider' },
    { kind: 'header', title: 'Informações' },
    {
      ...
      icon: <Celebration />,
      children: [
        { segment: 'criar_evento', title: eventoTitle, icon: <EditCalendar /> },
        { segment: 'eventos_postados', title: 'Eventos postados', icon: <Verified /> },
        { segment: 'eventos_criados', title: 'Eventos criados', icon: <Verified /> },
      ],
    },
    { segment: 'configuracoes', title: 'Configurações', icon: <Settings /> },
    { kind: 'divider' },
    { segment: 'home', title: 'Voltar à Home', icon: <ExitToApp /> },
  ];
  ...
  React.useEffect(() => {
    api.get<User>('/auth/dados', { withCredentials: true })
      .then(res => {
        setUser(res.data);
        setSession({
          user: { email: res.data.email, image: res.data.foto },
        });
      })
      .catch(_err => {
  ...
  function renderContent(pathname: string, router: Router) {
    switch (pathname) {
      ...
      case '/dashboard':
        return <BoasVindasFuncionario nome_usuario={user?.nome} router={router} />;
      case '/dashboard/info': return <InfoPessoal />;
      case '/dashboard/estabelecimento': return <Estabelecimento />;
      case '/dashboard/endereco': return <Endereco disabledComponents={false} />;
      case '/dashboard/contato': return <Contatos />;
      case '/dashboard/galeria': return <Galeria />;
      case '/eventos/eventos_postados': return <EventosPostadosFuncionario router={router} />;   
      case '/eventos/criar_evento': return <CriarEventoFuncionario setEventoTitle={setEventoTitle} />;
      case '/eventos/eventos_criados': return <EventosCriadosFuncionario />;
      case '/eventos/em_analise': return <ScreenDash />;
      case '/configuracoes': return <ScreenDash />;
      case '/home':
        n('/');
        return null;
      default: return <ScreenError />;
    }
  }
  ...
  return (
    ...
    >
      <DashboardLayout
        slots={{
          toolbarActions: () => <NotificacoesFuncionários router={router} />
        }}
      >
        <PageContainer>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
        ...
        <DialogActions>
          ...
          <Button color="primary" variant='contained' onClick={() => { logout(); setSession(null); }} sx={{ backgroundColor: 'var(--roxoForteDashboard)', color: 'white' }} href="/login">
            Sair
          </Button>
        </DialogActions>
      </Dialog>
